...
description: >-
  If a with Infinite Scroll enabled loads all items at once instead of
  paginating, the issue is typically related to layout configuration.
tags:
  - FlutterFlow
  - Troubleshooting
  - Widget
...
2. **Let ListView Handle Its Own Scrolling**
   ...
   - This allows `ListView` to control scroll behavior and calculate items to load per page.
      ...
      ![Infinite Scroll Pagination in ListView in FlutterFlow](../assets/20250430121248035007.png)
...
3. **Wrap ListView Inside a Fixed-Height Container (if nested)**
   If `ListView` is inside a scrollable parent (like `Column` or `ListView`), wrap it in a `Container` with a defined height (e.g., `500px`). This ensures it doesn't expand indefinitely.
      ...
      ![Infinite Scroll Pagination in ListView in FlutterFlow](../assets/20250430121248379992.png)
...
5. **Use Layout Structure That Supports Scroll Isolation**
   Allow `ListView` to scroll independently before the parent scroll takes over. Combine this with defined height and `Expanded` usage for best results.
      ...
      <video className="docs-video" autoPlay loop muted playsInline controls aria-label={"Configuring infinite-scroll pagination in a ListView"}><source src={require("../assets/20250430121249048672.mp4").default} type="video/mp4" /></video>
