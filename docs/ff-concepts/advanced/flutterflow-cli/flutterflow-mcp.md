...
- Anything outside the FlutterFlow project itself, such as running the app, deploying it, creating Firebase projects, managing secrets, or handling App Store submissions.
...
To roll back, use FlutterFlow's project version history in the visual builder, which is the same mechanism used for visual edits. Each agent push lands as a commit there with whatever commit message the agent supplied.
