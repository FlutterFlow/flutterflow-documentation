...
2. Also, set the **Update Delay (ms)**, which determines the time after the user has stopped typing for the page to refresh its UI. For example, if the *Update Delay (ms)* value is set to 2000 ms (2 seconds), the page will update 2 seconds after the user is done typing. For this example, let's set it to 0 ms.
