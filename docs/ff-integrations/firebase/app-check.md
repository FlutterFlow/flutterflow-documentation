...
5. You might want to see if it works on a real device or an emulator. To run on a real device, you can set the **Android Provider** to **Play Integrity** and to run on an emulator, set it to **Debug,** and then try checking it by downloading the APK.
    ...
    2. If it doesn't work for *Debug*, you can try [downloading the code](../../ff-concepts/advanced/ff-cli.md#download-projects), following the instructions [here](https://firebase.google.com/docs/app-check/flutter/debug-provider#android), and running it locally.
